...
$SUDO docker build -t vilight-appimage-builder-img-$DOCKER_SUFFIX \
...
(
    ...
    $SUDO docker run -it \
    -e GIT_REPO="$GIT_REPO" \
    --name vilight-appimage-builder-cont-$DOCKER_SUFFIX \
    -v $FRESH_CLONE_DIR:/opt/vilight \
    --rm \
    --workdir /opt/vilight/contrib/build-linux/appimage \
    vilight-appimage-builder-img-$DOCKER_SUFFIX \
    ./_build.sh $REV
